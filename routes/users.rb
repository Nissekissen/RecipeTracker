class MyApp < Sinatra::Application
  ...
  get '/profile/:id' do | id |
    ...
    if @tab == "bookmarks"
      @bookmarks = Recipe.join(:saved_recipes, recipe_id: :id).select(Sequel[:recipes][:id], :title, :description, :image_url, :url).where(user_id: @profile.id).group(:recipe_id).all
    elsif @tab == "groups"
